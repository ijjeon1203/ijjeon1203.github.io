...
async function mdToSvelte({
  ...
  let processorBeforeRehype = unified().use(remarkParse).use(emoji).use(remarkDirective).use(reserveSvelteCommands).use(reserveSvelteCommands$1).use(remarkFrontmatter).use(remarkExtractFrontmatter, { yaml: parse }).use(remarkGfm);
  remarkPlugins?.forEach((plugin) => {
    if (Array.isArray(plugin)) {
      const [p, options] = plugin;
      processorBeforeRehype = processorBeforeRehype.use(p, options);
    } else {
      processorBeforeRehype = processorBeforeRehype.use(plugin);
    }
  });
  if (highlighter) {
...
const sveltepress = ({
  ...
  const plugins = typeof theme?.vitePlugins === "function" ? theme.vitePlugins(corePlugin) : [
    theme?.vitePlugins,
    ...corePlugin
  ];
  if (addInspect)
